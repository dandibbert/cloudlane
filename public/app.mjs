...
const navigation = [ ['routes', '优选记录', 'route'], ['profiles', '配置方案', 'layers'], ['tunnels', 'Tunnel 资源', 'server'], ['credentials', 'API 凭据', 'key'], ['jobs', '执行任务', 'pulse'], ['activity', '操作记录', 'clock'], ['help', '使用指南', 'help'] ];
...
function routePage() {
  ...
  const stats = [
    ['优选记录', rows.length, '本地只保存映射与偏好', 'route', ''],
    ['配置就绪', ready, sslPending ? `${sslPending} 条 SSL 仍处理中 · 不阻断` : '当前远端配置核对通过', 'check', ''],
    ['等待前置', pending, '等待主机名 / Fallback / DNS', 'clock', 'pink'],
    ['需要关注', rows.filter(attention).length, '发现远端变化，不静默覆盖', 'pulse', 'amber']
  ];
  return html`${header('优选记录', 'Cloudflare 当前配置是事实源；面板保存管理映射，并持续同步 Tunnel、SaaS 与两侧 DNS。', html`${button('导入现有记录', 'import', 'download')}${button('新建优选记录', 'new-route', 'plus', 'btn primary')}`, 'EVERY ROUTE, IN SYNC')}${S.syncError ? notice(S.syncError, 'amber', '部分方案同步失败') : ''}${discovered.length ? html`<div class="demo-banner remote-discovery">${ic('download')}<span>最近同步发现 ${discovered.length} 条可识别、但尚未纳入管理的远端优选记录。</span>${button('查看并导入', 'import', 'arrow', 'text-btn')}</div>` : ''}<section class="stats" aria-label="记录概览">${stats.map(([title, count, hint, glyph, tone]) => html`<article class="stat ${tone}"><div class="stat-top">${title}${ic(glyph)}</div><div class="stat-bottom"><span class="stat-number">${fmtCount(count)}</span><span class="stat-unit">条</span></div><p class="stat-hint">${hint}</p></article>`)}</section><div class="connection-strip"><div class="strip-icon">${ic('spark')}</div><div class="strip-intro"><h3>一次编排，完整连接</h3><p>每一段路径，都由你掌控</p></div><div class="mini-flow">${[['globe', '访问域名'], ['spark', '优选入口'], ['cloud', 'SaaS 回源'], ['server', 'Tunnel 服务']].map(([glyph, label], i) => html`${i ? html`<span class="flow-arrow">${ic('arrow')}</span>` : ''}<span class="flow-node">${ic(glyph)}${label}</span>`)}</div></div><div class="section-heading"><h2>所有连接 <span>${rows.length}</span></h2>${button('配置导出', 'export', 'download', 'text-btn')}</div><div class="toolbar"><label class="search-box">${ic('search')}<input id="route-search" type="search" placeholder="搜索名称、域名或服务地址…" value="${S.search}" aria-label="搜索记录"><kbd>⌘ K</kbd></label><label class="filter-select"><select id="profile-filter" aria-label="按配置方案筛选"><option value="">所有配置方案</option>${S.data.profiles.map(p => html`<option value="${p.id}" ${yes(S.filterProfile === p.id)}>${p.name}</option>`)}</select>${ic('down')}</label><div class="tabs" role="group" aria-label="按状态筛选">${[['all', '全部', rows.length], ['ready', '已就绪', ready], ['attention', '待处理', rows.length - ready]].map(([key, label, count]) => html`<button class="tab ${S.tab === key ? 'active' : ''}" data-action="tab" data-id="${key}" aria-pressed="${S.tab === key}">${label}<em>${count}</em></button>`)}</div></div>${rows.length ? html`<div class="record-list">${filtered.length ? filtered.map(recordCard) : empty('没有匹配的记录', '试试另一个关键词，或清除筛选条件。', 'clear-filters', '清除筛选', 'search')}</div><div class="list-footer"><span>显示 ${filtered.length} / ${rows.length} 条记录</span><span>${ic('info')} 卡片内容来自最近一次 Cloudflare 同步；“就绪”不代表已测速或探测本地服务。</span></div>` : html`<div class="setup-steps">${[['1', '添加 API 凭据', '授权源账户和访问 Zone'], ['2', '创建配置方案', '选择 Tunnel、域名和入口'], ['3', '接入优选记录', '先预览，再确认执行']].map(([n, title, hint]) => html`<div class="setup-step"><span>${n}</span><div><strong>${title}</strong><small>${hint}</small></div></div>`)}</div>${empty('为你的第一条连接，开一条新路。', '可以从现有 Cloudflare 配置只读发现并导入，也可以创建全新的优选记录。', S.data.credentials.length ? S.data.profiles.length ? 'new-route' : 'new-profile' : 'new-credential', S.data.credentials.length ? S.data.profiles.length ? '新建优选记录' : '创建配置方案' : '添加 API 凭据')}`}`;
}
function recordCard(r, index) {
  ...
  return html`<article class="record-card ${attention(r) ? 'warning' : ''}"><div class="record-identity"><div class="service-icon ${index % 3 === 1 ? 'pink' : ''}">${ic('server')}</div><div><button class="record-title" data-action="detail" data-id="${r.id}">${r.name}</button><p class="service-url" title="${live.service}">${live.service}</p><div class="record-group">${ic('folder')}<span>${p?.name || '方案已移除'}</span><span class="divider">/</span><span>${p?.tunnelName || '未知 Tunnel'}</span></div></div></div><div class="domain-map"><span class="domain-label">MAIN</span><button class="domain-value main" title="查看连接详情" data-action="detail" data-id="${r.id}">${live.publicHostname}</button><span class="domain-label origin">ORIGIN</span><span class="domain-value">${live.originHostname}</span><span class="domain-label edge">EDGE</span><span class="domain-value">${live.edgeHostname}</span></div><div class="record-status">${job?.status === 'paused' ? jobBadge('paused') : routeBadge(status)}<span class="ssl-line" title="${o?.sslStatus || '尚未检查'}">${ic(o?.sslStatus === 'active' ? 'lock' : 'clock')}${sslText}</span></div><div class="record-actions">${iconButton('诊断与状态', 'detail', 'pulse', r.id)}${iconButton('编辑记录', 'edit-route', 'edit', r.id)}${iconButton('打开访问域名', 'visit', 'external', r.id)}</div></article>`;
}
